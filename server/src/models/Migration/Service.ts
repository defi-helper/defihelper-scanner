...
export class MigrationService {
  ...
  private async getMigrations(): Promise<MigrationHandlerList> {
    return new Promise((resolve, reject) => {
      readdir(this.dir, (err, files) => {
        if (err)
          return reject(new Error(`Migrations dir ${this.dir} not found`));
  ...
  async up() {
    ...
    const [completed, migrations] = await Promise.all([
      this.getCompleted(),
      this.getMigrations(),
    ]);
    const candidates = Object.entries(migrations).filter(
